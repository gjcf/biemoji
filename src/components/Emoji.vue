<template>
  <div :style="{ height: (height + 40) + 'px' }" class="tabs">
    <div class="tab-panel" v-if="hidden.indexOf('微信') == -1">
    ...
    <div class="tab-panel" v-if="hidden.indexOf('QQ') == -1">
    ...
    <div class="tab-panel" v-if="hidden.indexOf('酷安') == -1">
      <input type="radio" name="tab" :id="id + '-ka'" />
      <label class="tab-item" :for="id + '-ka'">酷安</label>
      <div class="tab-content">
        <div :style="{ height: height + 'px' }" class="emoji-container">
          <span class="emoji-wrapper" v-for="[key, value] of Object.entries(kaEmojis)" :key="value">
            <img :src="'https://unpkg.com/biemoji@latest/dist/ka/' + value + '.png'" :alt="value" @click="addEmoji(key)">
          </span>
        </div>
        <div class="delete-button" @click="deleteEmoji()" v-if="button">
          <svg height="24px" viewBox="0 0 24 24" width="24px" fill="#666666">
...
<script>
import { wxEmojiArray, qqEmojiArray, kaEmojiArray } from '../util'
